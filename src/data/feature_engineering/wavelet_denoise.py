...
def wavelet_denoise(
    ...
    level: int = 3,
    thr_mode: str = "soft",
    recon_mode: str = "symmetric",
    selected_level: Optional[int] = 0,
    method: str = "sqtwolog",
    ...
    denoiser = WaveletDenoising(
        ...
        method=method,  # closest match to global BM threshold
    )

    df = df.assign(denoised_signal=df[signal_column].apply(lambda x: denoiser.fit(x)))
...
class WaveletDenoising:
    ...
    def std(self, signal, level=None):
        ...
        if level > self.nlevel:
            ...
            print(
                " Level has been replaced by the largest possible\
                  value"
            )
            level = self.nlevel - 1
    ...
    def WavTransform(self, signal):
        ...
        if self.nlevel == 0:
            level = dwt_max_level(signal.shape[0], filter_len=self.filter_.dec_len)
            self.nlevel = level
    ...
    def Denoise(self, signal, coeffs):
        ...
        # WAVEDEC
        thr = [
            self.DetermineThreshold(coeffs[1 + level] / sigma[level], self.energy_perc)
            * sigma[level]
