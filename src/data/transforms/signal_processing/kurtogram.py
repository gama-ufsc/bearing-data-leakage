...
def plotKurtogram(Kwav, freq_w, nlevel, Level_w, Fs, fi, index):
    """
    Plots the kurtogram.
    ...
    plt.imshow(
        Kwav,
        aspect="auto",
        extent=(0, freq_w[-1], range(2 * nlevel)[-1], range(2 * nlevel)[0]),
        interpolation="none",
        cmap=plt.cm.hot_r,
    )
    ...
    plt.plot(Fs * fi, index, "yo")
    plt.xlabel("Frequency (Hz)")
    plt.ylabel("Level k")
    # plt.figtext(0.075, 0.90, "(a)", fontsize=15)
    plt.title(
        "Level %.1f, Bw=%.2f Hz, fc=%.2f Hz"
        % (
            np.round(10 * Level_w[index]) / 10,
            Fs * 2 ** (-(Level_w[index] + 1)),
            Fs * fi,
        )
    )
...
def Fast_Kurtogram(x, nlevel, verbose=False, Fs=1, NFIR=16, fcut=0.4, opt1=1, opt2=1):
    ...
    verbose : bool, optional
        If True, outputs debugging information (default is False)
    Fs : int, optional
        Sampling frequency of signal x (default is 1)
    NFIR : int, optional
        Length of FIR filter (default is 16)
    fcut : float, optional
        Fraction of Nyquist for filter (default is 0.4)
    opt1 : int, optional
        1: classical kurtosis based on 4th order statistics
        2: robust kurtosis based on 2nd order statistics of the envelope
        (default is 1)
    opt2 : int, optional
        1: the kurtogram is computed via a fast decimated filterbank
        2: the kurtogram is computed via the short-time Fourier transform
        (default is 1)
...
def K_wpQ(x, h, g, h1, h2, h3, nlevel, verbose, opt, level=0):
    ...
    level : int
        Decomposition level for this call
...
def K_wpQ_local(x, h, g, h1, h2, h3, nlevel, verbose, opt, level):
    ...
    tuple
        K : numpy array
            2-D vector containing the kurtosis value of the signal and the 2 kurtosis values corresponding to the signal filtered into 2 different band-passes
        KQ : numpy array
            2-D vector containing the 3 kurtosis values corresponding to the signal filtered into 3 different band-passes
...
def DBFB(x, h, g):
    ...
    tuple
        a : numpy array
            Approximation coefficients vector
        d : numpy array
            Detail coefficients vector
...
def binary(i, k):
    ...
    a = np.zeros(k)
    temp = i
    for expoent in np.arange(k - 1, -1, -1):
        a[k - expoent - 1] = int(np.fix(temp / 2**expoent))
        temp = temp - int(np.fix(a[k - expoent - 1] * 2**expoent))
...
# Kwav, Level_w, freq_w, c, f_lower, f_upper = Fast_Kurtogram(np.array(acceleration_raw[axis]),
# levels,
# verbose=True,
# Fs=fs)
